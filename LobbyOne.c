    ...
    while(getline(&bank, &bufferSize, fp) != -1)
    {
        ...
        for(ptrA = bank; *ptrA != '\0'; ptrA++)
        {
            ...
            for(ptrB = ptrA +1; *ptrB != '\0'; ptrB++)
            {
                ...
                int current = (*ptrA - '0') * 10 + (*ptrB - '0');

                if(current > maxBankJolts)
                {
                    maxBankJolts = current;
                }
            }
        }

        totalOutputJoltage += maxBankJolts;
    }
